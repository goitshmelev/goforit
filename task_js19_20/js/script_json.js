...
console.log('Входные данные: ', arrayData);

var skills = _.map(arrayData, 'skills');
var listSkills=[];

_.forEach(skills, function(key, value) {
    listSkills = _.concat(listSkills, skills[value]);
});

var finalSkills = _.sortBy(_.uniq(listSkills), function (value) {
                        return _.toLower(value);
                    });

console.log('Массив скиллов всех людей: ', finalSkills);

var names = _.map(arrayData, 'name');
var finalNames = _.map(_.sortBy(arrayData, ['friends' , 'name']), 'name');

console.log('Массив имен людей, отсортированных в зависимости от количества их друзей: ', finalNames);

var friends = _.map(arrayData, 'friends');
var listFriends=[];

_.forEach(friends, function(key,value) {
    listFriends=_.concat(listFriends, friends[value]);
});

console.log('Массив всех друзей всех пользователей, без повторений:', _.uniq(_.map(listFriends, 'name')));
